...
const SovereignDashboard = () => {
    ...
    const [installPath, setInstallPath] = (0, react_1.useState)('/data/data/com.termux/files/home/Feac-ultimate-sovereign/android');
    // Sidebar Menu Data
    const menuItems = [
    ...
    return (<div className="flex min-h-screen bg-[#020202] text-white font-sans overflow-hidden">
      ...
      <aside className="w-64 bg-[#080808] border-r border-white/5 flex flex-col shadow-2xl">
        ...
        <nav className="flex-1 p-4 space-y-2 mt-4">
          {menuItems.map((item) => (<button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all ${activeTab === item.id
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        {activeTab === 'dashboard' && (<div className="space-y-6 animate-in fade-in duration-500">
        ...
        {activeTab === 'emergent' && (<div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
        ...
        {activeTab === 'chat' && (<div className="flex flex-col h-full animate-in zoom-in-95 duration-500">
        ...
        {activeTab === 'settings' && (<div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-[#0f0f0f] border border-white/5 p-8 rounded-3xl max-w-2xl">
              ...
              <div className="mt-4 flex gap-3">
                <input value={installPath} onChange={(e) => setInstallPath(e.target.value)} className="flex-1 bg-black border border-white/10 rounded-xl px-4 py-3 text-xs font-mono text-emerald-500 focus:border-blue-500 outline-none transition-all"/>
