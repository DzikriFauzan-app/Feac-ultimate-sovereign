...
const ResourceMonitor = () => {
    ...
    const [currentStats, setCurrentStats] = (0, react_1.useState)({ ram: 0, cpu: 0, temp: 0 });
    (0, react_1.useEffect)(() => {
    ...
    return (<div className="p-4 bg-[#080808] border border-gray-900 rounded-2xl mt-4 shadow-inner">
      <div className="flex justify-between items-center mb-4">
        <h4 className="text-[10px] font-black text-gray-500 tracking-[0.3em] uppercase flex items-center gap-2">
          <lucide_react_1.Activity size={12} className="text-blue-500"/> System_Vital_Sign
        </h4>
      ...
      <div className="h-16 flex items-end gap-1 mb-4 px-1">
        {data.map((val, i) => (<div key={i} className="flex-1 bg-blue-600/40 border-t border-blue-400/50 rounded-t-sm transition-all duration-500" style={{ height: `${val}%` }}></div>))}
      </div>
