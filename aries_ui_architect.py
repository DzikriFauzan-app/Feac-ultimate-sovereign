...
def overhaul_dashboard():
    # Koreksi Case Sensitive berdasarkan screenshot user
    target_dir = "src/components/Dashboard"
    
    if not os.path.exists(target_dir):
        # Fallback search jika path masih meleset
        for root, dirs, files in os.walk("src"):
            if "Dashboard" in dirs or "dashboard" in dirs:
                target_dir = os.path.join(root, next(d for d in dirs if d.lower() == "dashboard"))
                break
    
    if not os.path.exists(target_dir):
        print("❌ Gagal menemukan folder Dashboard. Cek manual dengan 'ls src/components'")
        return

    print(f"🎯 Target dikunci: {target_dir}")

    # UI Code: Mewah, Mahal, Tab Emergent, GPT Chat, dan Navigasi Berfungsi
    sovereign_code = """
...
"""
    ...
    # Tulis file baru
    new_file_path = os.path.join(target_dir, "SovereignDashboard.tsx")
    with open(new_file_path, "w") as f:
        f.write(sovereign_code)
    
    print(f"✅ FINALIZED! Desain Mahal & Fungsional dibuat di: {new_file_path}")
    print("🚀 Sidebar Navigasi sudah aktif. Semua menu (Emergent, Chat, Settings) bisa dibuka satu per satu.")
