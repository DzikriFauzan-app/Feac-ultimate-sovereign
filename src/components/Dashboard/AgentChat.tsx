...
import { Send, UserCog, Sparkles } from 'lucide-react';
import { sfx } from '../../utils/SovereignSound';

const AgentChat = () => {
  const [target, setTarget] = useState('AssetAgent');
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      if (data.status === 'spawned') {
        sfx.playInitialize(); // Suara "Kelahiran" Agen
      }
  ...
  return (
    <div className="bg-[#050505] border border-gray-900 rounded-3xl p-5 mt-4 flex flex-col h-[450px] shadow-2xl">
      <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-900">
        <div className="flex items-center gap-2">
           ...
           <input 
             className="bg-transparent text-[10px] text-white font-black outline-none w-32 uppercase tracking-widest"
             value={target}
      ...
      <div className="flex-1 overflow-y-auto space-y-3 mb-4 pr-2 scrollbar-hide">
        {history.map((chat, i) => (
          <div key={i} className={`text-[10px] p-3 rounded-2xl ${
            chat.role === 'MASTER' 
            ? 'bg-blue-600/10 ml-10 border border-blue-500/20' 
            : 'bg-zinc-900/50 mr-10 border border-emerald-500/20'
          } ${chat.type === 'spawned' ? 'border-amber-500/50 bg-amber-500/5' : ''}`}>
      ...
      <div className="flex gap-2 bg-black p-1 rounded-2xl border border-gray-800">
        <input 
          ...
          placeholder="COMMAND_AGENT_..."
          className="flex-1 bg-transparent px-4 py-2 text-[10px] text-white outline-none"
        />
