...
import { Play, Activity, Save, RefreshCw } from 'lucide-react';
...
const Canvas = () => {
  ...
  return (
    <div 
      ...
      className="relative w-full h-[550px] bg-[#050505] bg-[radial-gradient(#1a1a1a_1px,transparent_1px)] [background-size:25px_25px] overflow-hidden rounded-2xl border border-gray-900 touch-none"
    >
      ...
      {nodes.map((node) => (
        <div 
          key={node.id}
          onTouchStart={() => handleGrab(node.id)}
          className={`absolute bg-[#0a0a0a]/90 backdrop-blur-md border-2 ${node.color} ${draggingNode === node.id ? 'scale-110 z-30 shadow-[0_0_30px_rgba(59,130,246,0.2)]' : 'z-10 shadow-xl'} p-3 rounded-xl w-36 transition-all duration-75`}
