    ...
    return (<div className="p-5 pb-24 space-y-6">
      <div className="bg-[#0a1612] border border-emerald-500/20 p-6 rounded-2xl relative overflow-hidden shadow-2xl">
        <h2 className="text-xl font-black text-white italic tracking-tighter uppercase flex items-center">
          <lucide_react_1.Activity className="mr-2 text-emerald-500 w-5 h-5 animate-pulse"/> NeoGrid Agents
        </h2>
      ...
      <div className="space-y-4">
        {agents.map(agent => (<div key={agent.id} className="bg-[#111821] border border-gray-800 p-5 rounded-2xl flex items-center justify-between group hover:border-emerald-500/30 transition-all">
            <div className="flex items-center space-x-4">
              <div className="p-3 bg-emerald-500/10 rounded-xl border border-emerald-500/20">
                <lucide_react_1.Cpu className="text-emerald-500 w-5 h-5"/>
              </div>
              <div>
      ...
      <div className="bg-black/50 border border-gray-800 p-4 rounded-2xl">
        ...
        <div className="flex space-x-2">
          ...
          <button className="bg-blue-600 p-2 rounded-lg text-white active:scale-90"><lucide_react_1.Zap className="w-4 h-4"/></button>
        </div>
      </div>
    </div>);
};
exports.default = NeoGrid;
