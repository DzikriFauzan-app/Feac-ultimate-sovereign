    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 bg-[#0d131a] border-r border-gray-800 z-[70] transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0 lg:static'}`}>
        ...
        <nav className="p-4 space-y-1 overflow-y-auto h-[calc(100vh-160px)] custom-scrollbar">
          <p className="px-4 text-[10px] font-black text-gray-700 uppercase tracking-widest mb-2">Modules</p>
          {menuItems.map((item) => (
          ...
          {userRole === 'OWNER' && (
            <button onClick={() => { setActiveTab('artifacts'); setIsOpen(false); }}
              ...
              <span className="text-xs font-black uppercase tracking-tighter">Artifacts Room</span>
            </button>
          )}

          <p className="px-4 text-[10px] font-black text-gray-700 uppercase tracking-widest mt-8 mb-2">Neural Links</p>
          <button onClick={() => { setActiveTab('brain'); setIsOpen(false); }}
            className={`w-full flex items-center space-x-3 p-4 rounded-2xl transition-all shadow-lg ${activeTab === 'brain' ? 'bg-purple-600 text-white shadow-purple-900/20' : 'bg-[#1a1421] text-purple-400 border border-purple-900/20'}`}>
