...
const Billing = ({ currentTier = 'OWNER' }) => {
    ...
    return (<div className="p-5 pb-24 space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        ...
        <div className="flex bg-[#111821] p-1 rounded-lg border border-gray-800">
          <button onClick={() => setIsYearly(false)} className={`px-3 py-1 text-[8px] font-black uppercase rounded-md transition-all ${!isYearly ? 'bg-blue-600 text-white' : 'text-gray-500'}`}>Monthly</button>
          <button onClick={() => setIsYearly(true)} className={`px-3 py-1 text-[8px] font-black uppercase rounded-md transition-all ${isYearly ? 'bg-blue-600 text-white' : 'text-gray-500'}`}>Yearly (-25%)</button>
        </div>
      </div>

      {currentTier === 'OWNER' && (<div className="bg-gradient-to-br from-[#1a1405] to-[#0a0f14] border border-yellow-600/30 rounded-2xl p-6 relative overflow-hidden shadow-2xl">
          ...
          <lucide_react_1.ShieldCheck className="absolute -right-6 -bottom-6 w-32 h-32 text-yellow-500 opacity-5 rotate-12"/>
        </div>)}
      ...
      <div className="space-y-4">
        ...
        <div className="flex space-x-4 overflow-x-auto pb-4 custom-scrollbar snap-x">
          {tiers.map((tier) => (<div key={tier.name} className={`min-w-[260px] snap-center bg-[#111821] border ${currentTier === tier.name ? 'border-blue-500 bg-[#161e29]' : 'border-gray-800'} rounded-2xl p-5 flex flex-col shadow-xl transition-all`}>
              <div className="flex justify-between items-start">
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{tier.name}</p>
                {tier.name === 'Platinum' && <lucide_react_1.Crown className="w-3 h-3 text-yellow-500"/>}
              </div>
              ...
              <div className="mt-6 space-y-3 flex-1">
                {tier.features.map(f => (<div key={f} className="flex items-center space-x-2 text-[11px] font-medium text-gray-400">
                    <lucide_react_1.Check className="w-3.5 h-3.5 text-blue-500 shrink-0"/>
                    <span>{f}</span>
                  </div>))}
              </div>
